...
#
# flags-2-env-web-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/flags-2-env/flags-2-env-web-server:dev --load .
#
#   # This crate resolves dependencies from git remotes. If any is
...
#     -t ghcr.io/flags-2-env/flags-2-env-web-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/flags-2-env/flags-2-env-web-server:dev .
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    cargo build --release --bin flags-2-env-web-server; \
    strip target/release/flags-2-env-web-server; \
    cp target/release/flags-2-env-web-server /usr/local/bin/flags-2-env-web-server
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/flags-2-env-web-server /usr/local/bin/flags-2-env-web-server
ENV FLAGS_2_ENV_WEB_BIND=0.0.0.0:8081
EXPOSE 8081
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/flags-2-env-web-server"]
...
COPY --from=builder /usr/local/bin/flags-2-env-web-server /usr/local/bin/flags-2-env-web-server
...
# The service exposes `/healthz` for liveness and `/readyz` for readiness.
...
#     ghcr.io/flags-2-env/flags-2-env-web-server:dev
#
