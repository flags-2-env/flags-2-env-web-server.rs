#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `flags-2-env-api-server`.
//!
...
//! to flags-2-env — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `flags-2-env-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `flags-2-env-api-server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `flags-2-env-api-server` cluster |
...
//! `flags-2-env-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `FLAGS_2_ENV_API_URL`, `FLAGS_2_ENV_NATS_URL`, and the rest; see the
/// table in `ores-transport`'s README.
pub const ENV_PREFIX: &str = "FLAGS_2_ENV";
...
pub const SERVICE_SLUG: &str = "flags-2-env";
