# The four avenues between `flags-2-env-web-server` and `flags-2-env-api-server`

`flags-2-env` follows the fleet-wide contract in
...
|---|------|------|-------------------|
| 1 | `direct_read` | `flags-2-env-web-server` → Postgres, via `flags-2-env-lib-core` | a read on the page's critical path |
| 2 | `http` | `flags-2-env-web-server` → `flags-2-env-api-server`, stateless | the default, and anything that writes |
| 3 | `tcp` | `flags-2-env-web-server` ⇄ `flags-2-env-api-server`, held open | chatty internal calls where the handshake dominates |
| 4 | `jet_stream` | `flags-2-env-web-server` → NATS → `flags-2-env-api-server` | work the caller does not need to watch finish |

All four carry the same envelope and land on the same handler, so a
`flags-2-env-e2e` suite can drive one operation four ways and assert the answers
...
|---|---|
| Slug | `flags-2-env` |
| Environment prefix | `FLAGS_2_ENV_` |
| Request subject | `flags-2-env.api.operation.v1` |
| Result subject | `flags-2-env.api.operation-result.v1` |
| Durable consumer | `flags-2-env-api-server-v1` |
...
|----------|--------|--------------|
| `FLAGS_2_ENV_READONLY_DATABASE_URL` | 1 | no direct reads |
| `FLAGS_2_ENV_API_URL` | 2 | no stateless HTTP |
| `FLAGS_2_ENV_API_MTLS_ADDR` | 3 | no stateful TCP |
| `FLAGS_2_ENV_API_TLS_SERVER_NAME` | 3 | plaintext to a TLS-terminating mesh |
| `FLAGS_2_ENV_WEB_CLIENT_CERT_FILE` | 3 | ” |
| `FLAGS_2_ENV_WEB_CLIENT_KEY_FILE` | 3 | ” |
| `FLAGS_2_ENV_API_CA_FILE` | 3 | ” |
| `FLAGS_2_ENV_NATS_URL` | 4 | no asynchronous avenue |

A missing variable disables its avenue. A variable that is present and unsafe
fails startup: a plaintext `FLAGS_2_ENV_API_URL` off loopback, a
`FLAGS_2_ENV_NATS_URL` that is not `tls://`, or mutual TLS with two of its
four files set.

In-cluster, `FLAGS_2_ENV_NATS_URL` points at the messaging service defined in
...
**In `flags-2-env-web-server`** — implement `DirectReader` over `flags-2-env-lib-core`:
...
impl DirectReader<Operation, Projection> for LibCoreReader {
        ...
        -> Result<Projection, TransportError>
    {
        ...
        // not an access check.
        // Then build the query with flags-2-env-lib-core's query builders.
    }
...
**In `flags-2-env-api-server`** — implement `OperationHandler` once, and hand the same
...
**Avenue 1 is read-only twice over.** `flags-2-env-lib-core` gates its write surface
behind a `read-write` cargo feature and migrations behind `migrate`. Depend on
it from `flags-2-env-web-server` as:

```toml
flags-2-env-lib-core = { git = "…", default-features = false, features = ["read-only"] }
```

so a mutating call is a *compile error*, not a code-review miss. Separately,
`FLAGS_2_ENV_READONLY_DATABASE_URL` must name a Postgres role without
`INSERT`, `UPDATE`, `DELETE` or DDL. A mistake has to defeat both. Migrations
belong to `flags-2-env-lib-core` and `declarative-migrations`, never to a web server.
